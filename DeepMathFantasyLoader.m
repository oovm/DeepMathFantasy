...
(* ::Text::GrayLevel[0.5]:: *)
(* Autogenerated DeepMathFantasy loader file *)


Unprotect[DeepMathFantasy`PackageScope`Private`$TopLevelLoad];
DeepMathFantasy`PackageScope`Private`$TopLevelLoad=MemberQ[$ContextPath, "Global`"];


BeginPackage["DeepMathFantasy`"];


ClearAll[DeepMathFantasy];
DeepMathFantasy::usage="DeepMathFantasy is a manager head for the DeepMathFantasy package";
...
DeepMathFantasy["Directory"]:=
  $PackageDirectory;
$PackageDirectory=
  DirectoryName@$InputFileName;


DeepMathFantasy["Name"]:=
  $PackageName;
$PackageName=
  "DeepMathFantasy";
...
DeepMathFantasy["LoadingParameters"]:=$PackageLoadSpecs
$PackageLoadSpecs=
  Merge[
    {
      With[
...
DeepMathFantasy["PackageListing"]:=$PackageListing;
$PackageListing=<||>;
DeepMathFantasy["Contexts"]:=$PackageContexts;
If[!ListQ@$PackageContexts,
  $PackageContexts=
    {
      "DeepMathFantasy`",
      "DeepMathFantasy`PackageScope`Private`",
      "DeepMathFantasy`PackageScope`Package`"
      }
  ];
$PackageDeclared=
...
DeepMathFantasy["FEScopedSymbols"]:=$PackageFEHiddenSymbols;
$PackageFEHiddenSymbols={};
DeepMathFantasy["PackageScopedSymbols"]:=$PackageScopedSymbols;
...
DeepMathFantasy["AllowRescoping"]:=$AllowPackageRescoping;
$AllowPackageRescoping=
  Replace[
    Lookup[$PackageLoadSpecs, "AllowRescoping"],
    Except[True|False]->$TopLevelLoad
    ];
DeepMathFantasy["AllowRecoloring"]:=$AllowPackageRecoloring;
$AllowPackageRecoloring=
  Replace[
    Lookup[$PackageLoadSpecs, "AllowRecoloring"],
    Except[True|False]->$TopLevelLoad
    ];
DeepMathFantasy["AllowAutocompletions"]:=$AllowPackageAutocompletions;
...
DeepMathFantasy["FileContexts"]:=$PackageFileContexts;
...
DeepMathFantasy["DeclaredPackages"]:=$DeclaredPackages;
...
DeepMathFantasy["LoadedPackages"]:=$LoadedPackages;
...
PackageFileContext[f_String?DirectoryQ]:=
  With[{s=PackageFileContextPath[f]},
    StringRiffle[Append[""]@Prepend[s,DeepMathFantasy],"`"]
    ];
...
DeepMathFantasy::nodep="Couldn't load dependency `` of type ``";
DeepMathFantasy::nodup="Couldn't update dependency `` of type ``";
  ...
  Block[{retcode, site, path, file, tmp},
    ...
    site=
      ...
      If[retcode<300,
        ...
        DeleteFile[tmp];
        file,
        Message[DeepMathFantasy::nodep, dep, "Package"];
        DeleteFile[tmp];
        $Failed
        ]
      ];
    ...
    Monitor[
      MapThread[
        Check[
          ...
          Message[DeepMathFantasy::nodep, #2, "Paclet"];
          $Failed
          ]&,
        {
          pacs,
      ...
      Monitor[
        MapThread[
          Check[
            ...
            Message[DeepMathFantasy::nodup, #2, "Paclet"];
            $Failed
            ]&,
          {
            StringDelete[deps, "`"],
...
$PackageErrorMessage=
  "DeepMathFantasy encountered exception ``";
...
PackageThrowMessage[
  ...
  PackageThrowMessage[
    MessageName[DeepMathFantasy, tag],
    body,
    ops
    ];
PackageThrowMessage[
...
PackageCatchMessage[
  ...
  Replace[
    Thread[Map[Hold[MessageName[DeepMathFantasy, #]]&, msg], Hold],
    {
      {}:>
        Check[expr, 
...
PackageFailureException[
  ...
  (
    Set[MessageName[DeepMathFantasy, tag], body];
    PackageFailureException[
      MessageName[DeepMathFantasy, tag],
      body,
      ops
...
PackageAttachMainAutocomplete[]:=
  PackageAddAutocompletions[DeepMathFantasy, 
    Table[
      Replace[{}->None]@
        Cases[
          DownValues[DeepMathFantasy],
          Nest[
            Insert[#, _, {1, 1, 1}]&,
            (HoldPattern[Verbatim[HoldPattern]][
              DeepMathFantasy[s_String, ___]
              ]:>_),
            n-1
            ]:>s,
          Infinity
...
PackagePrepPackageSymbol[]:=
  Switch[$AllowPackageSymbolDefinitions,
    None,
      Remove[DeepMathFantasy],
    False,
      Clear[DeepMathFantasy],
    _,
      PackageAttachMainAutocomplete[]
    ]
...
If[
  (Clear@DeepMathFantasy`PackageScope`Private`$loadAbort;!#)&@
    DeepMathFantasy`PackageScope`Private`$loadAbort,
  Unprotect[DeepMathFantasy`PackageScope`Private`$LoadCompleted];
  DeepMathFantasy`PackageScope`Private`$LoadCompleted=True;
  DeepMathFantasy`PackageScope`Private`PackagePostProcessContextPathReassign[]
  ]