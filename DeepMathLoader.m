...
(* ::Text::GrayLevel[0.5]:: *)
(* Autogenerated DeepMath loader file *)


Unprotect[DeepMath`PackageScope`Private`$TopLevelLoad];
DeepMath`PackageScope`Private`$TopLevelLoad=MemberQ[$ContextPath, "Global`"];


BeginPackage["DeepMath`"];


ClearAll[DeepMath];
DeepMath::usage="DeepMath is a manager head for the DeepMath package";
...
DeepMath["LoadingParameters"]:=$PackageLoadSpecs
$PackageLoadSpecs=
  Merge[
    {
      With[
...
DeepMath["PackageListing"]:=$PackageListing;
$PackageListing=<||>;
DeepMath["Contexts"]:=$PackageContexts;
If[!ListQ@$PackageContexts,
  $PackageContexts=
    {
      "DeepMath`",
      "DeepMath`PackageScope`Private`",
      "DeepMath`PackageScope`Package`"
      }
  ];
$PackageDeclared=
...
DeepMath["FEScopedSymbols"]:=$PackageFEHiddenSymbols;
$PackageFEHiddenSymbols={};
DeepMath["PackageScopedSymbols"]:=$PackageScopedSymbols;
...
DeepMath["FileContexts"]:=$PackageFileContexts;
...
DeepMath["DeclaredPackages"]:=$DeclaredPackages;
...
DeepMath["LoadedPackages"]:=$LoadedPackages;
...
PackageFileContext[f_String?DirectoryQ]:=
  With[{s=PackageFileContextPath[f]},
    StringRiffle[Append[""]@Prepend[s,DeepMath],"`"]
    ];
...
DeepMath::nodep="Couldn't load dependency `` of type ``";
DeepMath::nodup="Couldn't update dependency `` of type ``";
  ...
  Block[{retcode, site, path, file, tmp},
    ...
    site=
      ...
      If[retcode<300,
        ...
        DeleteFile[tmp];
        file,
        Message[DeepMath::nodep, dep, "Package"];
        DeleteFile[tmp];
        $Failed
        ]
      ];
    ...
    Monitor[
      MapThread[
        Check[
          ...
          Message[DeepMath::nodep, #2, "Paclet"];
          $Failed
          ]&,
        {
          pacs,
      ...
      Monitor[
        MapThread[
          Check[
            ...
            Message[DeepMath::nodup, #2, "Paclet"];
            $Failed
            ]&,
          {
            StringDelete[deps, "`"],
...
$PackageErrorMessage=
  "DeepMath encountered exception ``";
...
PackageCatchMessage[
  ...
  Replace[
    Thread[Map[Hold[MessageName[DeepMath, #]]&, msg], Hold],
    {
      {}:>
        Check[expr, 
...
PackageFailureException[
  ...
  (
    Set[MessageName[DeepMath, tag], body];
    PackageFailureException[
      MessageName[DeepMath, tag],
      body,
      ops
...
PackageAttachMainAutocomplete[]:=
  PackageAddAutocompletions[DeepMath, 
    Table[
      Replace[{}->None]@
        Cases[
          DownValues[DeepMath],
...
If[
  (Clear@DeepMath`PackageScope`Private`$loadAbort;!#)&@
    DeepMath`PackageScope`Private`$loadAbort,
  Unprotect[DeepMath`PackageScope`Private`$LoadCompleted];
  DeepMath`PackageScope`Private`$LoadCompleted=True;
  DeepMath`PackageScope`Private`PackagePostProcessContextPathReassign[]
  ]